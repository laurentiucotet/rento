...
export function TicketDialog({ open, onOpenChange, propertyId, onTicketAdded, ticketToEdit }: TicketDialogProps) {
  ...
  const [costEstimateRange, setCostEstimateRange] = useState([0, 5000])
  ...
  useEffect(() => {
    if (ticketToEdit) {
      ...
      setCostEstimateRange([0, ticketToEdit.costEstimate])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const ticketData: Ticket = {
      ...
      costEstimate: costEstimateRange[1] - costEstimateRange[0],
      urgency,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="costEstimate">Cost Estimate Range ($0 - $5000)</Label>
            <Slider
              min={0}
              max={5000}
              step={100}
              value={costEstimateRange}
              onValueChange={(value) => setCostEstimateRange(value as [number, number])}
            />
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>${costEstimateRange[0]}</span>
              <span>${costEstimateRange[1]}</span>
            </div>
          </div>
